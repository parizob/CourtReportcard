...
/**
 * If proofread flagged an exact capitalization/spelling fix once, clone it for
 * every other exact `original` hit in the document (all entries), each with its
 * own start/end + context anchors. Does not auto-accept — reporter still reviews.
 *
...
export function expandExactRepeatAnnotations(entries, annotations) {
  ...
  // One seed per original+suggestion+type across the whole document.
...
/**
 * Immediate doubled time markers — mechanically checkable. Covers identical
 * pairs with or without periods: "p.m. p.m.", "pm pm", "a.m a.m", "am. am.", etc.
 * The proofread prompt's a.m./p.m. FP guardrail makes the model unreliable on
 * these, so we inject them in code (same idea as filterPhantomFixes /
 * speaker-label typos: don't leave a string-equality bug to prompt luck).
 * Suggestion keeps one copy in the same form as written (no style rewrite).
 */
const DOUBLED_TIME_MARKER_RE = /(?<![A-Za-z0-9])([ap]\.?m\.?)\s+\1(?![A-Za-z0-9])/gi

export function detectDoubledTimeMarkers(entries) {
  const flags = []
  for (const entry of entries || []) {
    const text = entry?.text || ''
    if (!text) continue
...
/** Append deterministic doubled a.m./p.m. flags (deduped against existing). */
export function mergeDoubledTimeMarkerAnnotations(entries, annotations) {
  const base = Array.isArray(annotations) ? annotations : []
...
"extra_word" — A word appears twice, or an extraneous word is present that shouldn't be there. Example (always flag): "p.m. p.m." or "a.m. a.m." — identical time markers repeated back-to-back with no clock digits between them; suggestion is a single "p.m." or "a.m.".
...
- Do NOT flag "a.m." or "p.m." as "extra_word", "missing_word", missing text, or a steno artifact when they appear as a normal time-of-day marker. That includes (1) a marker next to a clock time or hour in the same phrase ("12:45 p.m.", "9 a.m.", "3 p.m.", "9:41 a.m. Eastern Time", "The time is 4:20 p.m."), (2) a marker on an exhibit-mark, entered-into-the-record, or identification line when the clock digits are on the same entry, including the preceding or following line ("RESPONDENT EXHIBIT 12" then "11:42 a.m."), and (3) off-the-record / on-the-record parentheticals ("(Off the record at 4:12 p.m.)"). Do NOT invent a missing clock time. This exemption covers a single time marker ONLY.
- DO flag an immediate doubled time marker with no clock digits between the copies — "p.m. p.m." or "a.m. a.m." — as "extra_word", severity critical. Suggestion: a single "p.m." or "a.m." (keep one copy). The time-marker exemption above does NOT protect doubled junk.
- Do NOT flag "though" as a spelling or steno substitution for "thought" when it is a conjunction or discourse adverb (e.g., "even though he had been told", "So even though the agreement was signed", "the evidence will show, though, we think"). Still flag a clear wrong-word on that same sentence where "thought" is the verb and "though" cannot be a conjunction (e.g., "I though I locked the door").
- Do NOT expand a spoken contraction into its uncontracted form. "it's", "that's", "there's", "we're", "they're", "he's", "she's", and "who's" written as spoken are correct. Do not suggest "it is", "that is", "there is", "there are", "we are", etc. This does NOT exempt a missing apostrophe ("thats" → "that's") or the its/it's homophone when the possessive/contraction choice is wrong on the face of that entry.
- Do NOT flag a "Q." or "A." prefix as "extra_word" or a redundant speaker tag when it follows an examination heading such as "BY MS. BELL:", "BY MR. PRATT:", "DIRECT EXAMINATION", or "CROSS-EXAMINATION". That pairing is required format in many jurisdictions. Still flag a doubled tag on the same line ("Q. Q.") and still check spelling, apostrophes, and homophones in the question or answer text.
- Do NOT flag capitalization on the first word of a short standalone Q or A after a speaker tag ("A. Absolutely.", "A. Correct.", "A. Yes.", "Q. Okay."). That word starts a new answer or question and is correctly capitalized. Still flag a mid-sentence capital that is not "I" or a proper noun, and still flag missing apostrophes inside the same answer ("A. Dont.").
- Do NOT rewrite a verb into a different tense or participle when the form on the page is already grammatical for its construction. In particular, do NOT change a base-form verb after "to" (infinitive: "to provide") or after a supporting "do"/"does"/"did" (e.g., "Did ComEd provide…") into a past participle ("provided") or other tensed form. Do NOT classify those as spelling or steno substitutions. Still flag clear wrong-word substitutions (homophones, misspellings) and genuine subject-verb disagreement that is wrong on the face of that same entry.
...
- EXTRA_WORD DOUBLED-WORD EXEMPTION: A doubled word where each instance is a grammatically distinct token (e.g., "that that," "had had," "is, is") must NEVER be flagged as "extra_word" duplication. This exemption does NOT apply to identical time markers repeated back-to-back ("p.m. p.m.", "a.m. a.m.") — those are always "extra_word" critical.
...
- Proofread entries labeled "CAPTION", "CERTIFICATE", "HEADING", "INDEX", and "EXHIBITS" with the same spelling, grammar, homophone, and steno-error checks as testimony. These pages are filed with the court and often contain the same class of typos (e.g., "da0y" for "day", "FRIEDN" for "FRIEND").
- On CAPTION / CERTIFICATE / HEADING / INDEX / EXHIBITS entries: fill-in blanks are intentional and correct. Do NOT flag underscores ("_", "__", "___", longer runs), empty brackets ("[]", "[ ]"), blank date/signature lines, or placeholder time slots (e.g. "00 HOURS:00 MINUTES") as missing words, extra words, or incomplete sentences. Do NOT invent words to fill blanks (including "DIRECT", dates, or names). Do NOT suggest replacing "[]" with underscores or the reverse. On INDEX / EXHIBITS specifically: also do NOT flag page numbers, dotted leaders (....), column alignment, or table-of-contents formatting as errors. Still flag clear typos in the surrounding filled-in words (e.g. "da0y" → "day", "FRIEDN" → "FRIEND").
...
export async function extractTranscriptWithGemini(fileOrText, mimeType) {
  ...
  // Runtime reference date — mirrors buildProofreadReferenceDateBlock in prompts.ts
  const proofreadRefDate = new Intl.DateTimeFormat('en-CA', {
    timeZone: 'America/New_York',
    year: 'numeric',
    month: '2-digit',
    day: '2-digit',
  }).format(new Date())
  const proofreadRefBlock =
    `Reference date for this review: ${proofreadRefDate} (America/New_York). ` +
  ...
  // Deterministic: doubled a.m./p.m. (model often skips after FP guardrail).
  annots = mergeDoubledTimeMarkerAnnotations(entries, annots)
  annots.forEach((a, i) => { a.id = i + 1 })
