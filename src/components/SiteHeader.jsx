...
export default function SiteHeader() {
  ...
  // Bell notification state — only for unauthenticated users
  const [bellRinging, setBellRinging] = useState(false)
  const [hasNotification, setHasNotification] = useState(false)
  const [notifOpen, setNotifOpen] = useState(false)
  const notifRef = useRef(null)

  // Account dropdown state (authenticated)
    ...
    <nav className="sticky top-0 z-50 bg-[#f8f9fa]">
      <div className="flex justify-between items-center w-full px-8 h-[65px]">
        ...
        <div className="flex items-center gap-4">
          ...
          {/* Bell — promo notification for unauthenticated */}
          {!isAuthenticated && (
          ...
          {/* Authenticated: bell (static) + account dropdown */}
          {isAuthenticated && (
            <>
                ...
                {accountOpen && (
                  <div className="absolute right-0 top-[calc(100%+8px)] w-56 bg-surface-container-lowest rounded-xl editorial-shadow border border-outline-variant/20 overflow-hidden py-2">
                    <div className="px-4 py-3 border-b border-outline-variant/10">
                      ...
                      <p className="text-[10px] text-on-surface-variant mt-0.5">Free Plan</p>
                    </div>
                    <Link
                      to="/dashboard"
          ...
          {/* Unauthenticated: account icon opens sign-in modal */}
          {!isAuthenticated && (
            <button
