#!/usr/bin/env node
/**
 * Upload transcript_07_dense_50pages.txt to Dev and kick analyze-case.
 *
 * Usage:
 *   SEED_EMAIL='you@…' SEED_PASSWORD='…' node scripts/run-dense-dev-upload.mjs
 *
 * Uses VITE_SUPABASE_* from .env (must be Dev). Charges ~50 tokens.
 * Polls until analyzed / deleted / timeout, then prints case id + status.
 */
...
const plainText = readFileSync(
  join(__dirname, 'test-transcripts/transcript_07_dense_50pages.txt'),
  'utf8',
)

// Mirror analyze-case countPages: page number alone on a line, right-ish.
function countPages(text) {
  const re = /^\s{10,}(\d+)\s*$/gm
  let max = 0
...
console.log(`Transcript pages (counted): ${pages}, bytes: ${plainText.length}`)
...
const caseName = `Dense 50p stress ${new Date().toISOString().slice(0, 16)}`
...
const fileName = 'transcript_07_dense_50pages.txt'
...
while (Date.now() < deadline) {
  ...
  if (c?.status === 'analyzed') {
    console.log('\nDONE analyzed')
    console.log('Score with: node scripts/score-dense-dev-case.mjs', caseRow.id)
    process.exit(0)
  }
